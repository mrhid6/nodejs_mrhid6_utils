...
class Config {
    ...
    init(opts) {
        ...
        if (this._options.useExactPath == false) {
            this._options.configDirectory = path.join(
                this._options.configBaseDirectory,
                this._options.configName
            );
        } else {
            this._options.configDirectory = path.resolve(
                this._options.configBaseDirectory
            );
        }

        const configFile = `${this._options.configName}.${this._options.configType}`;
        this._options.configFilePath = path.join(
            this._options.configDirectory,
            configFile
        );
    }
    ...
    validateBeforeLoad() {
        ...
        if (
            fs.existsSync(this._options.configFilePath) == false &&
            this._options.createConfig == false
        ) {
            return false;
        }
    ...
    parseIniDataFile() {
        try {
            return ini.parse(
                fs.readFileSync(this._options.configFilePath, "utf-8")
            );
        } catch (error) {
            throw error;
        }
