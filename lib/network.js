...
class Network {
    ...
    triggerEvent(socket, event, data) {
        ...
        if (this._eventHandlers[event] != null) {
            ...
            if (this._debugTriggeredEvents) {
                console.log(
                    "Called NetworkEvent:" + event + " But not registered!"
                );
            }
        }
    }
    ...
    createResponsePacket(Packet, data) {
        const name = Packet.getPacketName()
            .replace("packet.server.", "")
            .replace("packet.client.", "");
        const newName = name + ".res";
        const newPacket = this.createPrefixedPacket(
            newName,
            data,
            Packet.getPacketSocket()
        );
        return newPacket;
    }
    ...
    sendPacket(Packet) {
        ...
        if (socketId == null) {
            ...
            this._io
                .of(namespace)
                .to(socketId)
                .emit(Packet.getPacketName(), Packet.getPacketData());
        }
    }
