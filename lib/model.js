...
class ModelSave {
    constructor(DBColumnName, Value) {
...
class Model {
    ...
    setAndSave(key, value, force = false) {
        ...
        if (force == false && this.get(key) == value) {
            return;
        }

        this.set(key, value);
        const translation = this._options.translations.find(t => t[1] == key)

        if (translation != null) {
            const DBColumnName = translation[0];
            const SaveModelData = new ModelSave(DBColumnName, value);
    ...
    SaveToDB() {
        return new Promise((resolve, reject) => {
            ...
            this._savedata.forEach(SaveData => {
                const DBData = [SaveData.getValue()]
                const Update = `${SaveData.getColumnName()}=?`
