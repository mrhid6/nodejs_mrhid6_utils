...
class ModelTranslation {
    constructor(DBColumnName, VariableName, JsonName, Type, DefaultValue = null) {
        this._columnName = DBColumnName;
...
class ModelLink {
    ...
    constructor(VariableName, JsonName, TableName, Linkage, ModelClass, Type) {
        this._name = VariableName;
        this._jsonname = JsonName;
...
class Model {
    ...
    _ProcessData(data, index = -1) {
        this._options.translations.forEach(translation => {
            ...
            const JsonName = translation.getJsonName();
            ...
            try {
                DataValue = index > -1 ? data[index][DBColumnName] : data[DBColumnName];
            } catch (err) {}

            if (DataValue == null) {
                try {
                    DataValue = index > -1 ? data[index][JsonName] : data[JsonName];
                } catch (err) {}
    ...
    BuildLink(link) {
        return new Promise((resolve, reject) => {
            const DataPath = link.getName();
            const DBTable = link.getTableName()
            const Linkage = link.getLinkage();
            const LinkModelClass = link.getModelClass();
            const LinkType = link.getType() || TYPES.SINGLE;
            ...
            this._DB.query(SQL).then(rows => {
                ...
                Promise.all(promises).then(Models => {
                    ...
                    if (Models.length > 0 && LinkType == TYPES.SINGLE) {
    ...
    DeleteFromDB() {
        return new Promise((resolve, reject) => {
            ...
            const promises = [];
            this._options.links.forEach(link => {
                promises.push(this.DeleteLinkFromDB(link));
            })

            Promise.all(promises).then(() => {
                const SQL = `DELETE FROM ${this._options.table} WHERE ${search}`
                return this._DB.query(SQL, QueryTypes.EXEC)
            }).then(() => {
                resolve();
            })
    ...
    toJson() {
        ...
        this._options.translations.forEach(translation => {
            ...
            const DataValue = this.get(`${TranslationName}`);

            if (TranslationType == TYPES.DATE || TranslationType == TYPES.TIMESTAMP) {
                resData[`${JsonName}`] = DataValue.getTime();
            } else if (TranslationType == TYPES.BOOLEAN) {
                resData[`${JsonName}`] = (DataValue ? 1 : 0);
            } else {
                resData[`${JsonName}`] = DataValue;
            }
        })

        this._options.links.forEach(link => {
            const DataPath = link.getName();
            const JsonName = link.getJsonName()
            const DBTable = link.getTableName()
            const Linkage = link.getLinkage();
            const LinkModelClass = link.getModelClass();
            const LinkType = link.getType() || TYPES.SINGLE;

            const DataValue = this.get(`${DataPath}`);

            if (DataValue != null) {
                if (LinkType == TYPES.SINGLE) {
                    resData[`${JsonName}`] = DataValue.toJson();
                } else {
                    resData[`${JsonName}`] = [];
                    for (let i = 0; i < DataValue.length; i++) {
                        const d = DataValue[i];
                        resData[`${JsonName}`].push(d.toJson())
                    }
                }
            } else {
                if (LinkType == TYPES.SINGLE) {
                    resData[`${JsonName}`] = null;
                } else {
                    resData[`${JsonName}`] = [];
                }
            }
    ...
    fromJson(data) {
        ...
        this._options.links.forEach(link => {
            ...
            if (LinkType == TYPES.MANY) {
                LinkData = [];
                for (let i = 0; i < DataValue.length; i++) {
                    const modelData = DataValue[i];
                    const iModel = new LinkModelClass();
                    iModel.fromJson(modelData);

                    LinkData.push(iModel);
                }
            } else {
                const iModel = new LinkModelClass();
                iModel.fromJson(DataValue);
                LinkData = iModel;
            }

            this.set(`${DataPath}`, LinkData);
        })
    }
